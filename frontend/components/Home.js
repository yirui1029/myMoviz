...
function Home() {
 ...
 const movies = decouvertMovies.map((data, i) => {
   ...
   const overviewformatte=data.overview
   if(overviewformatte.length>250){
    overviewformatte=overviewformatte.substring(0,250)+'...';

   }
   return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={overviewformatte} poster={urlImg} voteAverage={data.vote_average} voteCount={data.vote_count} />;
 });
